<script lang="ts">
	...
	const variantStyles: Record<AlertVariant, { wrapper: string; iconClass: string; svg: string }> = {
		info: {
			wrapper:
				'bg-sky-50 dark:bg-sky-950/40 border-sky-200 dark:border-sky-800/50 text-sky-900 dark:text-sky-200',
		...
		success: {
			wrapper:
				'bg-green-50 dark:bg-green-950/40 border-green-200 dark:border-green-800/50 text-green-900 dark:text-green-200',
...
{#if !dismissed}
	...
	>
		...
		<div class="flex-1 min-w-0">
			...
			{/if}
			{#if hasItems}
				<ul class={['list-disc space-y-1 pl-4 text-xs leading-relaxed opacity-90', message || title ? 'mt-2' : '']}>
