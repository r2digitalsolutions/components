<script lang="ts">
	...
	interface FeatureListProps {
		...
		/** Brand tint (default) or gray circles */
	...
	const icons: Record<NonNullable<FeatureListItem['icon']>, string> = {
		...
		spark:
			'M12 3v3m0 12v3M3 12h3m12 0h3M5.6 5.6l2.1 2.1m8.6 8.6l2.1 2.1M18.4 5.6l-2.1 2.1M7.7 16.3l-2.1 2.1'
	};
</script>
...
<div class={['space-y-3 w-full', className]}>
	...
	<Stack gap="md">
		{#each items as item (item.id)}
			<div class={['gap-3 flex', item.description ? 'items-start' : 'items-center']}>
				<span
					class={[
						'h-9 w-9 rounded-xl flex shrink-0 items-center justify-center',
						iconTone === 'neutral'
				...
				<div class={item.description ? 'min-w-0' : 'min-h-9 min-w-0 flex flex-col justify-center'}>
